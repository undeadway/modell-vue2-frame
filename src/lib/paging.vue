<template>
	<div class="mv2-paging-box">
		...
		<div v-if="pageData.total <= 10">
			<div>
				<div v-for="n in pageData.total" :class="checkIsNow(n) ? 'mv2-is-now' : ''" :key="n">{{ n }}</div>
			</div>
		</div>
		<div v-else>
			<!--
			...
			<div v-else-if="pageData.page > pageData.total - 5">
				...
				<div :class="checkIsNow(pageData.total - 4) ? 'mv2-is-now' : ''" @click="onChangePage(pageData.total - 4)">{{ pageData.total - 4 }}</div>
				<div :class="checkIsNow(pageData.total - 3) ? 'mv2-is-now' : ''" @click="onChangePage(pageData.total - 3)">{{ pageData.total - 3 }}</div>
				<div :class="checkIsNow(pageData.total - 2) ? 'mv2-is-now' : ''" @click="onChangePage(pageData.total - 2)">{{ pageData.total - 2 }}</div>
				<div :class="checkIsNow(pageData.total - 1) ? 'mv2-is-now' : ''" @click="onChangePage(pageData.total - 1)">{{ pageData.total - 1 }}</div>
				<div :class="checkIsNow(pageData.total) ? 'mv2-is-now' : ''">{{ pageData.total }}</div>
			</div>
			<!--
			...
			<div v-else>
				...
				<div class="mv2-is-now" @click="onChangePage(pageData.page)">{{ pageData.page }}</div>
				<div @click="onChangePage(pageData.page + 1)">{{ pageData.page + 1}}</div>
				<div @click="onChangePage(pageData.page + 2)">{{ pageData.page + 2}}</div>
				<div @click="onNext()">&gt;&gt;</div>
				<div :class="checkIsNow(pageData.total) ? 'mv2-is-now' : ''" @click="onChangePage(pageData.total)">{{ pageData.total }}</div>
			</div>
		</div>
	</div>
</template>
